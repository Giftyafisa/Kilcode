...
def run_migration():
    """Fix betting_status_check constraint to be case-insensitive"""
    try:
            ...
            with db_engine.connect() as conn:
                ...
                conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS betting_codes_new (
                        ...
                        CONSTRAINT betting_status_check CHECK (LOWER(status) IN ('pending', 'won', 'lost', 'analyzing', 'approved', 'rejected'))
                    )
                """))
